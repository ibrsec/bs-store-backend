 ...
 */
  ...
  create: async (req, res) => {
    const { categoryId,title,description,price,brand, thumbnail,       /*after - not required*/  discountPercentage, rating, stock, images} = req.body;
    if (!categoryId || !title || !description || !price || !brand || !thumbnail) {
      res.status(400);
      throw new Error("Bad request - categoryId,title,description,price,brand, thumbnail fields are required!");
    }

    const newProduct = await Product.create({ categoryId,title,description,price,brand, thumbnail, discountPercentage, rating, stock, images });
 ...
 */
  ...
  update: async (req, res) => {
    ...
    const { modifiedCount } = await Product.updateOne(
      { _id: req.params.id },
      { categoryId,title,description,price,brand, thumbnail,       /*after - not required*/  discountPercentage, rating, stock, images },
      { runValidators: true }
    );
    if (!modifiedCount) {
      res.status(500);
 ...
 */
  ...
  patchUpdate: async (req, res) => {
    ...
    const { categoryId,title,description,price,brand, thumbnail, discountPercentage, rating, stock, images} = req.body;
    if (!(categoryId || title || description || price || brand || thumbnail)) {
      res.status(400);
      throw new Error("Bad request - At least one filed is required -> categoryId, title, description, price, brand, thumbnail, discountPercentage, rating, stock, images.");
    }
    ...
    const { modifiedCount } = await Product.updateOne(
      { _id: req.params.id },
      { categoryId,title,description,price,brand, thumbnail, discountPercentage, rating, stock, images},
      { runValidators: true }
    );
    if (!modifiedCount) {
      res.status(500);
